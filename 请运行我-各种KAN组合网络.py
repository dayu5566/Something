...
'''
    ...
    此程序可以实现单输出和多输出的回归。只需要改66行的n_out参数即可。n_out=1时，默认最后一列为输出，n_out=2时，默认最后2列为输出，以此类推。
'''
...
n_out = 1 #选择最后1列作为输出，如果等于2，则选择最后2列作为输出
...
hidden_dim = 120  # Feed Forward层（Attention后面的全连接网络）的隐藏层的神经元数量。该值越大，网络参数量越多，计算量越大。
num_layers = 2  #层数
