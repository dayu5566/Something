...
bp = MLPRegressor(hidden_layer_sizes=(32,),  max_iter=100, random_state=1, verbose=2)
# hidden_layer_sizes=(32,) 设置了一个隐藏层，其中有32个神经元。
# max_iter=100 指定最大迭代次数为100，即训练过程最多进行100个epoch。
# random_state=10 设置了一个随机数种子，以保证每次运行代码时结果的可重复性。
# verbose=2 设置了日志输出的详细程度，2表示输出更详细的训练过程信息。

model = bp.fit(vp_train, vt_train)
...
for ii in range(n_out):
    ...
    # 设置y轴标签为"值"，字体大小为5。
    if n_out == 1:     #如果是单步预测
        plt.title(f"The prediction result of BP :\nMAPE: {mape(Ytest[:, ii], predicted_data[:, ii])} %")
    else:
        plt.title(f"{ii+1} step of BP prediction\nMAPE: {mape(Ytest[:,ii], predicted_data[:,ii])} %")
