...
def bigru_model():
    ...
    bigru = Bidirectional(GRU(50, activation='selu', return_sequences=False))(inputs)
...
for ii in range(n_out):
    ...
    # 设置y轴标签为"值"，字体大小为5。
    if n_out == 1:     #如果是单步预测
        plt.title(f"The prediction result of BiGRU :\nMAPE: {mape(Ytest[:, ii], predicted_data[:, ii])} %")
    else:
        plt.title(f"{ii+1} step of BiGRU prediction\nMAPE: {mape(Ytest[:,ii], predicted_data[:,ii])} %")
