...
def gru_model():
    ...
    # 创建模型的输入层。Input用于定义输入的形状，这里的形状是由vp_train的第二维和第三维确定的。

    gru = GRU(128, activation='selu', return_sequences=False)(inputs)
    # 添加一个GRU层。GRU是一种循环神经网络层，这里设置了128个神经元，激活函数为'selu'。
    # return_sequences=False表示GRU层只返回最后一个时间步的输出。

    outputs = Dense(vt_train.shape[1])(gru)
    # 创建一个全连接层（Dense），神经元数量与vt_train的列数相同，即输出的维度。

    model = Model(inputs=inputs, outputs=outputs)
    # 创建一个模型，指定其输入和输出。

    model.compile(loss='mse', optimizer='Adam')
    # 编译模型。这里使用均方误差（mse）作为损失函数，优化器为Adam。

    model.summary()
    # 输出模型的总结信息，这可以展示模型的架构、参数数量等信息。

    return model
    # 返回构建的模型。

model = gru_model()
# 调用gru_model函数来建立GRU模型。
...
for ii in range(n_out):
    ...
    # 设置y轴标签为"值"，字体大小为5。
    if n_out == 1:     #如果是单步预测
        plt.title(f"The prediction result of gru :\nMAPE: {mape(Ytest[:, ii], predicted_data[:, ii])} %")
    else:
        plt.title(f"{ii+1} step of gru prediction\nMAPE: {mape(Ytest[:,ii], predicted_data[:,ii])} %")
