...
num_samples = 1000  # 可以设定从数据中取出多少个点用于本次网络的训练与测试。
...
def tcn_RNN_model():
    ...
    model = Sequential()
    model.add(inputs)
    model.add(TCN(nb_filters=128, kernel_size=3, dilations=[1, 2, 4], return_sequences=True))
...
for ii in range(n_out):
    ...
    # 设置y轴标签为"值"，字体大小为5。
    if n_out == 1:     #如果是单步预测
        plt.title(f"The prediction result of TCN-RNN :\nMAPE: {mape(Ytest[:, ii], predicted_data[:, ii])} %")
    else:
        plt.title(f"{ii+1} step of TCN-RNN prediction\nMAPE: {mape(Ytest[:,ii], predicted_data[:,ii])} %")
