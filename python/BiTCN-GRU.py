...
def Bitcn_gru_model():
    ...
    model.add((GRU(50, activation='selu', return_sequences=False)))
    model.add(Dense(10))
...
for ii in range(n_out):
    ...
    # 设置y轴标签为"值"，字体大小为5。
    if n_out == 1:     #如果是单步预测
        plt.title(f"The prediction result of BiTCN-GRU :\nMAPE: {mape(Ytest[:, ii], predicted_data[:, ii])} %")
    else:
        plt.title(f"{ii+1} step of BiTCN-GRU prediction\nMAPE: {mape(Ytest[:,ii], predicted_data[:,ii])} %")
