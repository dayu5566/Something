...
def cnn_gru_attention_model():
    # 定义一个包含CNN, gru和注意力机制的模型
    ...
    gru_out = GRU(128, return_sequences=True)(reshaped)  # LSTM层
    attention_out = attention_layer(gru_out, time_steps=reshaped.shape[1])  # 注意力机制层
...
for ii in range(n_out):
    ...
    # 设置y轴标签为"值"，字体大小为5。
    if n_out == 1:     #如果是单步预测
        plt.title(f"The prediction result of CNN-GRU-Attention :\nMAPE: {mape(Ytest[:, ii], predicted_data[:, ii])} %")
    else:
        plt.title(f"{ii+1} step of bagging prediction\nMAPE: {mape(Ytest[:,ii], predicted_data[:,ii])} %")
