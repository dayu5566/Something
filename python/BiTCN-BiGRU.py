...
def Bitcn_bigru_model():
    ...
    model = Sequential()
    model.add(inputs)
    model.add(Bidirectional(TCN(nb_filters=64, kernel_size=2, dilations=[1, 2, 4], return_sequences=True)))
    model.add(Bidirectional((GRU(50, activation='selu', return_sequences=False))))
    model.add(Dense(10))
...
# 调用bigru_model函数来建立BiGRU模型。
# 绘制BiTCN-BiGRU的模型结构
''''
    如果显示pydot没有安装等错误，请参考：
    https://blog.csdn.net/QAQIknow/article/details/119188790
    如果实在解决不了，就把下述绘图代码注释掉即可！
'''
tf.keras.utils.plot_model(
    model,
    to_file='BiTCN-BiGRU-model.png',
    show_shapes=True,
    show_dtype=True,
...
for ii in range(n_out):
    ...
    # 设置y轴标签为"值"，字体大小为5。
    if n_out == 1:     #如果是单步预测
        plt.title(f"The prediction result of BiTCN-BiGRU :\nMAPE: {mape(Ytest[:, ii], predicted_data[:, ii])} %")
    else:
        plt.title(f"{ii+1} step of BiTCN-BiGRU prediction\nMAPE: {mape(Ytest[:,ii], predicted_data[:,ii])} %")
