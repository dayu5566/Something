...
# 使用pandas模块的read_excel函数读取名为"CEEMDAN.xlsx"的文件。
...
def cnn_bigru_model():
    ...
    # 添加一个一维卷积层，有32个过滤器，核大小为2，激活函数为'relu'
    ...
    # 添加一个双向GRU层。Bidirectional包装器将GRU层应用于输入的正向和反向。
    # 设置了128个神经元，激活函数为'selu'。return_sequences=False表示该层只返回最后一个时间步的输出。

    outputs = Dense(vt_train.shape[1])(bigru)
    # 创建一个全连接层（Dense），神经元数量与train_y的列数相同，即输出的维度。

    model = Model(inputs=inputs, outputs=outputs)
    # 创建一个模型，指定其输入和输出。

    model.compile(loss='mse', optimizer='Adam')
    # 编译模型，使用均方误差（mse）作为损失函数，优化器为Adam。
...
for vmd_num in  range (0,len(values_vmd[0])):
    ...
    imf = imf.reshape(-1,1)
    combined_data = np.hstack((values[:,0:-1],imf)) # 把CEEMDAN分解的每列数据与原始数据特征列组合到一起
...
for ii in range(n_out):
    ...
    # 设置y轴标签为"值"，字体大小为5。
    if n_out == 1:     #如果是单步预测
        plt.title(f"The prediction result of CEEMDAN-CNN-BiGRU :\nMAPE: {mape(actual_test[:, ii], pre_test[:, ii])} %")
    else:
        plt.title(f"{ii+1} step of CEEMDAN-CNN-BiGRU prediction\nMAPE: {mape(actual_test[:,ii], pre_test[:,ii])} %")
