...
# 调用相关库
from mealpy import FloatVar,GWO,HHO,WOA,PSO,BMO,RIME,IWO,TPO  #mealpy库是智能算法的集合库，整理了最新的Version: 3.0.1   pip install mealpy==3.0.1
# 更多关于Mealpy库请看：https://mealpy.readthedocs.io/en/latest/
# 注意，请不要删除FloatVar，这不是智能优化算法，而是一个转换函数。
# 通过调用不同的算法即可，具体有多少算法，可以参考链接看到有多少个算法可以替换！：https://mp.weixin.qq.com/s?__biz=MzkyNDUwMjc3Mg==&mid=2247487689&idx=1&sn=f2c5a3020b7af0b296979b75d75207ad&chksm=c1d58f52f6a20644551f144d47f8877d7d4a70049b28a32a85038023b658eacf35419dc082f2#rd
...
def cnn_bilstm_attention_model():
    ...
    reshaped = Reshape((-1, 32 * maxpooling.shape[1]))(maxpooling)  # 重塑形状以适应LSTM层
    lstm_out = Bidirectional(LSTM(8, return_sequences=True))(reshaped)  # LSTM层
...
history = model.fit(vp_train, vt_train, batch_size=32, epochs=30, validation_split=0.25, verbose=2)
...
for ii in range(n_out):
    ...
    # 创建一个图形对象，并设置大小为10x3英寸，分辨率为250dpi。
    ...
    # 设置y轴标签为"值"，字体大小为5。
    x_major_locator = MultipleLocator(int(vt_test.shape[0] / 20))
    # 把x轴的刻度间隔设置为1，并存在变量里
    ...
    if n_out == 1:     #如果是单步预测
        plt.title(f"The prediction result of CNN-BiLSTM-Attention :\nMAPE: {mape(Ytest[:, ii], predicted_data[:, ii])} %")
        plt.savefig('优化前预测结果图.png')
    else:
        plt.title(f"{ii+1} step of CNN-BiLSTM-Attention prediction\nMAPE: {mape(Ytest[:,ii], predicted_data[:,ii])} %")
        plt.savefig(f"优化前预测结果图：第{ii+1}步 .png")
...
'''*****************************以上为标准CNN-BiLSTM-Attention程序*******************************************'''


'''*****************************接下来采用优化算法优化CNN-BiLSTM-Attention*****************************************'''
...
def fun(pop):
    ...
    kernel_size0 = int(pop[2])  # CNN核大小
    num_epochs = int(pop[3])#训练次数
    ...
    # 定义一个包含CNN, LSTM和注意力机制的模型
    inputs = Input(shape=(vp_train.shape[1], vp_train.shape[2]))
    conv1d = Conv1D(filters=filters0, kernel_size=kernel_size0, activation='relu')(inputs)
    maxpooling = MaxPooling1D(pool_size=2)(conv1d)
    reshaped = Reshape((-1, filters0 * maxpooling.shape[1]))(maxpooling)  # 重塑形状以适应LSTM层
    lstm_out = Bidirectional(LSTM(hidden_nodes0, return_sequences=True))(reshaped)  # LSTM层
    attention_out = attention_layer(lstm_out, time_steps=reshaped.shape[1])  # 注意力机制层
    attention_flatten = Flatten()(attention_out)  # 展平输出以适应全连接层
    outputs = Dense(vt_train.shape[1])(attention_flatten)  # 全连接层
    ...
    # model.summary()
    model.fit(vp_train, vt_train, epochs=num_epochs, batch_size=32, validation_data=(vp_test, vt_test),
              verbose=0)
    ...
    test_pred = m_out.inverse_transform(yhat)  # 反归一化

    MApe_error = mape(Ytest, test_pred)
    # 计算预测值和实际值之间的均方误差。

    return MApe_error,test_pred
    # 返回计算得到的均方误差。
def Objfun(x):  #
    MApe_error,test_pred = fun(x)
    return MApe_error

''' Objfun->目标函数, lb->下限, ub->上限 '''
problem_dict = {
    "obj_func": Objfun,
    "bounds": FloatVar(lb=[0.001,8,2,30], ub=[0.01,60,4,80]), # 分别对应学习率，神经元，CNN核大小,迭代次数,注意CNN核大小设置的时候不能超过每个vp_train的行数！也就是不能超过n_in
    "minmax": "min",
}
...
epoch = 10 # 最大迭代次数
pop_size = 10 # 种群数量
##需要替换算法的小伙伴，直接按  Ctrl+R，将本代码的  PSO 全部替换为别的，比如 GWO，WOA算法等
''' 第一种方式，需：from mealpy.swarm_based import PSO,HHO '''
PSO_model = PSO.OriginalPSO(epoch=epoch, pop_size=pop_size)

'''求解'''
PSO_best = PSO_model.solve(problem_dict)
# 保存优化结果
savemat('optimized_paras.mat', {'trace': PSO_model.history.list_global_best_fit , 'bestx': PSO_best.solution, 'result': PSO_best.target})
...
plt.plot(x,PSO_model.history.list_global_best_fit ,'r-' ,linewidth=1,label = 'PSO',marker = "x",markersize=8)
plt.xlabel('Iterations', fontsize=10)
# 设置x轴标签为“迭代次数”。
plt.ylabel('fitness', fontsize=10)
# 设置y轴标签为“适应度值”。
plt.title("Iterative curve of optimization algorithm")
plt.savefig('优化算法收敛曲线.png')
plt.ioff()  # 关闭交互模式
plt.show()# 显示绘制的图形。



print("最佳优化参数，学习率、最佳神经元,CNN核大小，epoch训练次数的参数分别为：", [int(PSO_best.solution[i]) if i > 0 else PSO_best.solution[i] for i in range(len(PSO_best.solution))])
...
pop=loadmat('optimized_paras.mat')['bestx'].reshape(-1,)

MApe_error,Optimized_test_pred = fun(pop)


savemat('prdict_results.mat',{'Standard_prediction':predicted_data,'Optimized_prediction':Optimized_test_pred})


mse_dic,rmse_dic, mae_dic, mape_dic, r2_dic, Optimized_table = evaluate_forecasts(Ytest, Optimized_test_pred, n_out)
# 调用evaluate_forecasts函数。
# 传递实际值(inv_y)、预测值(inv_yhat)以及预测的步数(n_out)作为参数。
# 此函数将计算每个预测步长的RMSE、MAE、MAPE和R2值。
...
print('优化后的预测结果指标：')
print(Optimized_table)#显示预测指标数值
...
Standard_prediction=loadmat('prdict_results.mat')['Standard_prediction']
Optimized_prediction=loadmat('prdict_results.mat')['Optimized_prediction']
...
for ii in range(n_out):
    ...
    # 创建一个图形对象，并设置大小为10x3英寸，分辨率为250dpi。
    plt.figure(figsize=(10, 3), dpi=250)
    x = range(1, len(Optimized_prediction) + 1)
    # 创建x轴的值，从1到实际值列表的长度。

    # 设置刻度标签的字体大小。
    plt.plot(x, Optimized_prediction[:, ii], linestyle="--", linewidth=0.5, label='Optimized_prediction')
    # 绘制预测值的折线图，线型为虚线，线宽为0.5，标签为'predict'。

    plt.plot(x, Standard_prediction[:, ii], linestyle="-.", linewidth=0.5, label='Standard_prediction')
    # 绘制预测值的折线图，线型为-.虚线，线宽为0.5，标签为'predict'。

    plt.plot(x, Ytest[:, ii], linestyle="-", linewidth=0.5, label='Real')
    # 绘制实际值的折线图，线型为直线，线宽为0.5，标签为'Real'。


    plt.xticks(x[::int((len(Optimized_prediction)+1))])
    # 设置x轴的刻度，每几个点显示一个刻度。
    plt.tick_params(labelsize=5)  # 改变刻度字体大小
    ...
    plt.ylabel("value", fontsize=5)
    # 设置y轴标签为"值"，字体大小为5。
    x_major_locator = MultipleLocator(int(len(Optimized_prediction)/ 20))
    # 把x轴的刻度间隔设置为1，并存在变量里
    ...
    if n_out == 1:     #如果是单步预测
        plt.title(f"The prediction result of CNN-BiLSTM-Attention")
        plt.savefig('优化前后预测结果对比图.png')
    else:
        plt.title(f"{ii+1} step of CNN-BiLSTM-Attention prediction")
        plt.savefig(f"优化前后预测结果对比图：第{ii+1}步 .png")
    # plt.xlim(xmin=600, xmax=700)  # 显示600-1000的值   局部放大有利于观察
    # 如果需要，可以取消注释这行代码，以局部放大显示600到700之间的值。
